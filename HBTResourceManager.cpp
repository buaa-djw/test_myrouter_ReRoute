...
std::vector<int> HBTResourceManager::collectAllFreeHBTs() const { std::vector<int> out; for (const auto& r : reservations_) if (!r.occupied) out.push_back(r.hbt_id); return out; }
