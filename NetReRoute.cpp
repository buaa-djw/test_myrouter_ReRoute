...
namespace {
enum class CandidateType{kEdgeRelocation,kRipupOneSink,kHBTSwap,kHBTInsert,kHBTRemove,kCrossDieRipupViaHBT};
struct Candidate{CandidateType type;int sink_pin=-1,sink_node=-1,old_parent=-1,new_parent=-1,new_hbt=-1;bool force=false;std::vector<RoutedSegment> segs;};
...
std::string editTypeToString(CandidateType t){ if(t==CandidateType::kCrossDieRipupViaHBT) return "cross_die_detour_via_hbt"; return "other"; }
}
...
static std::vector<Candidate> generateCrossDieDetourCandidatesImpl(const RouterDB& db, const HybridGrid& grid, const Net& net, const NetRouteResult& result, int sink_pin_index, int sink_tree_node, HBTResourceManager& hbt_manager, const CriticalNetOptimizer::Params& params, CriticalNetOptimizer::OptimizationStats* stats){
    std::vector<Candidate> out;
    if (sink_tree_node < 0 || sink_tree_node >= (int)result.tree_nodes.size()) return out;
    const RoutedPoint sink_point = result.tree_nodes[sink_tree_node].point;
    if (!result.is_3d) { if (stats) stats->rejected_by_cross_die_not_supported++; return out; }
    ...
    if(parents.empty()){ if (stats) stats->rejected_by_cross_die_not_supported++; return out; }
    std::sort(parents.begin(),parents.end());
    int parent_limit=std::max(1,std::min(params.beam_width, params.max_iterations_per_net));
    ...
    for(int i=0;i<(int)parents.size()&&i<parent_limit;++i){
        ...
        auto free=hbt_manager.collectFreeHBTsNear(grid, Point2D{sink_point.x,sink_point.y}, params.max_hbt_candidates_per_branch);
        if(free.empty()){ if (stats) stats->rejected_by_no_free_hbt++; continue; }
        int hlimit=std::min((int)free.size(), params.max_hbt_candidates_per_branch);
        for(int k=0;k<hlimit;++k){
            ...
            Candidate c; c.type=CandidateType::kCrossDieRipupViaHBT; c.sink_pin=sink_pin_index; c.sink_node=sink_tree_node; c.old_parent=old_parent; c.new_parent=p; c.new_hbt=free[k]; c.segs={a,b};
            out.push_back(c);
...
CriticalNetOptimizer::OptimizationStats CriticalNetOptimizer::optimize(std::vector<NetRouteResult>& results) const {
    ...
    for(int ridx:idx){ ++st.visited_nets; auto it=n2i.find(results[ridx].net_name); if(it==n2i.end()) continue; const Net& net=db_.nets[it->second]; auto& result=results[ridx];
        ...
        if(params_.enable_cross_die_ripup || params_.enable_cross_die_detour){
            auto gen = generateCrossDieDetourCandidatesImpl(db_, grid_, net, result, sink_pin, sink_node, hbt_manager_, params_, &st);
        ...
        for(auto &c:cands){ ++st.tried_candidates;
            ...
            std::vector<RoutedSegment> nseg; nseg.insert(nseg.end(),w.segments.begin(),w.segments.begin()+ob); nseg.insert(nseg.end(),c.segs.begin(),c.segs.end()); nseg.insert(nseg.end(),w.segments.begin()+ob+oc,w.segments.end()); w.segments.swap(nseg); sn.parent_index=c.new_parent; sn.incoming_segment_count=(int)c.segs.size(); sn.incoming_hbt_count=0; sn.incoming_hbt_res=sn.incoming_hbt_cap=0; sn.assigned_hbt_id=-1; for(auto&s:c.segs){if(s.uses_hbt){ if(hbt_manager_.isOccupied(s.hbt_id)){st.rejected_by_hbt_conflict++; goto NEXTC;} sn.incoming_hbt_count++;sn.assigned_hbt_id=s.hbt_id;sn.incoming_hbt_res+=db_.hbt.parasitic_res;sn.incoming_hbt_cap+=db_.hbt.parasitic_cap;}}
            ...
            bool force_ok=(c.type==CandidateType::kCrossDieRipupViaHBT)&&(params_.debug_force_accept_cross_die_ripup||params_.debug_force_accept_cross_die_detour);
            if(timing || force_ok){best=w;improved=true;++st.accepted_candidates; if(c.type==CandidateType::kCrossDieRipupViaHBT){st.accepted_cross_die_ripup_candidates++; if(force_ok){st.force_accepted_cross_die_ripup_candidates++; if(params_.verbose) std::cout<<"[reroute] force accept "<<editTypeToString(c.type)<<" net="<<net.name<<"\n";} hbt_manager_.reserve(c.new_hbt, net.name, -1, c.sink_node, c.sink_pin);} break;} else st.rejected_by_delay++; }
            NEXTC: ;
        }
