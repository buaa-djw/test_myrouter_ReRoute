...
public:
    ...
    struct Snapshot
    {
        std::vector<HBTReservation> reservations;
        Stats stats;
    };
    ...
    bool rebuildFromRouteResults(const std::vector<NetRouteResult>& results,
                                 std::ostream* os = nullptr);

    std::vector<int> collectFreeHBTsNear(const HybridGrid& grid,
                                         const Point2D& p,
                                         int max_count) const;
    std::vector<int> collectAllFreeHBTs() const;

private:
    bool hasValidSlot(int hbt_id) const;
